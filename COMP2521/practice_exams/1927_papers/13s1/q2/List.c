...
{
    int *array = malloc(L->nnodes*sizeof(int)); 
    int i = 0, j;  
    Link curr = L->first;
    
    //insert linked list values into array
    while (curr != NULL) {
        array[i] = curr->value;
        i++;
        curr = curr->next;
    ...
    curr = L->first;
    //insert array values in reverse back into the linked list
    for (j = L->nnodes-1; j >=0; j--) {
        curr->value = array[j];
        curr = curr->next;
    }
}
