...
{
	assert(g != NULL);
	assert(start >= MIN_MAP_LOCATION && start <= MAX_MAP_LOCATION && end >= MIN_MAP_LOCATION && end <= MAX_MAP_LOCATION);
	int nconnections = 0;
	int arrayIndex = 0;
	int i, j;
	//sorting through transport types
	for (i = 0; i <= MAX_TRANSPORT; i++) {
		if (inVList(g->connections[start], end, i)) {
			nconnections++;
            type[arrayIndex] = i;
            arrayIndex++;
        }
        // places connected via the same sea
        else if (i == BOAT) {
            for (j = MIN_MAP_LOCATION; j <= MAX_MAP_LOCATION; j++) {
                if (idToType(j) == SEA) {
                    if (inVList(g->connections[start], j, BOAT) && inVList(g->connections[end], j, BOAT)) {
                        nconnections++;
                        type[arrayIndex] = i;
                        arrayIndex++;
                        break;
