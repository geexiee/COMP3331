...
{
    int lower, higher, middle, result, flag = 0;
    lower = MIN_MAP_LOCATION;
    higher = MAX_MAP_LOCATION;
    middle = (lower + higher)/2;
    while (flag < 2) {
        result = strcmp(name,places[middle].name);
        if (result == 0) {
        ...
        if (middle == higher || middle == lower) {
            // edge case when name is the last name in the list
            if (flag > 0 && middle == lower && middle != higher) {
                middle = higher;
            }
            flag++;
        }
