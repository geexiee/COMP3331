...
{
//If the list is empty, insert the number as the only node
	if (L->first == NULL) {
...
//If the first node is larger than the number, the new number should be inserted first
	else if(L->first->data >= v) {
	    struct IntListNode *temp = L->first, *new_first_node;
		new_first_node = newIntListNode(v);
	    L->first = new_first_node;
		new_first_node->next = temp;
		L->size++;
	    return;
	}
//If the last node is smaller than the number itself
	if (L->last->data <= v) {
		struct IntListNode *new_last_node;
		new_last_node = newIntListNode(v);
		L->last->next = new_last_node;
		L->last = L->last->next;
		L->size++;
		return;
	}
//Check for if the new number should be between 2 numbers in the list
	struct IntListNode *current = L->first;
	while (current != NULL) {
		if (current->data <= v && current->next->data >= v) {
			struct IntListNode *temp, *new_node;
			new_node = newIntListNode(v);
			temp = current->next;
