...
{
	assert(L != NULL); 
	DLListNode *new_node = newDLListNode(it), *temp;
	//case for if current is first
	if (L->curr->prev == NULL) {
...
void DLListAfter(DLList L, char *it)
{
	assert(L != NULL);
	DLListNode *new_node = newDLListNode(it), *temp;
	//case for if current node is last
	if (L->curr->next == NULL) {
...
{
	assert (L != NULL);
	DLListNode *previous_node = NULL, *next_node = NULL, *temp = NULL;
	//case for if current is the only item
	if (L->curr->next == NULL && L->curr->prev == NULL) {
		L->curr = NULL;
		L->nitems--;
		return;
	}
	//case for if current is last
	else if (L->curr->next == NULL && L->curr->prev != NULL) {
		previous_node = L->curr->prev;
		temp = L->curr;
		free(temp);
		L->curr = previous_node;
