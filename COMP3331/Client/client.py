...
Instructions: python3 client.py 127.0.0.1 [portnumber]
Always run with 127.0.0.1 since its hardcoded into the server
If something goes wrong, try diff port nums or restart terminal

this client implements multithreading so that it has one listening thread
(for processing messages from the server) and one sending thread
(for taking user input, doing basic checking, and sending messages to the server)
each thread is implemented through a loop which constantly receives input or server messages

the two threads communicate with each other through global variables when needed
the listening thread is crucial for shutting down whenever any other client sends SHT
the sending thread is the only one set as daemon so that when listening thread shuts down
so does the sending thread.
'''
...
class ListeningThread(threading.Thread):
	...
	def run(self):
		...
		''' loop which just constantly receives and processes packets from the server
			reads the 'operation' field first to determine which operation is being processed
			then processes it on this side accordingly'''
		while True:
			try:
			...
			# if the above loop fails, that means the other thread has been closed by SHT so this one needs to exit too
			except:
				sys.exit()
				
# thread class for taking input from user and sending messages to the server	
